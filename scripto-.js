                ...
                return new Promise(function(resolve, reject) {
                    if (input instanceof Blob) {
                        ...
                        reader.onload = function() {
                            ...
                            // LZMA.compress(new Uint8Array(reader.result), 9, (resulta, error) => {
//             if (error) {
//                 reject(error);
//             } else {
// //             saveCompressedFile(compressedData, filename);
// //         resolve(new Uint8Array(result));
// 	const filename="compressed_filex.lzma";
//     const blobx = new Blob([new Uint8Array(resulta)], { type: "application/octet-stream" });
...
//         });
                                ...
                                /////////////////////////
                               //  LZMA.compress(decompressed, 9, (resultb, error) => {
//             if (error) {
//                 reject(error);
//             } else {
// //             saveCompressedFile(compressedData, filename);
// //         resolve(new Uint8Array(result));
// 	const filename="compressed_file2x.lzma";
//     const bloby = new Blob([new Uint8Array(resultb)], { type: "application/octet-stream" });
...
//                                 var zz = LZMA.decompress(byteArray);
//                                 var vvv = LZMA.decompress(uint8Array);
//                                 var www = LZMA.decompress(reader.result);

// LZMA.decompress(uint8Array,(resultw, error) => {
//             if (error) {
//                 reject(error);
//             } else {
//             var decompresx =resultw;
// 			}
// }, (percent) => {});
//                                 LZMA.decompress(uint8Array, function(res) {
...
//                                 });
								var decompressed=LZMA.decompress(uint8Array);
                                resolve(decompressed);
                            } catch (error) {
                                reject(error);
                            }
...
m.on('zoomend', function() {
    ...
    } else //   //m.addLayer(district_point);
